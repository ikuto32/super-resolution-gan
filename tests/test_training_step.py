...
class TinyGenerator(nn.Module):
    ...
    def forward(self, lr: torch.Tensor, target_size) -> dict[str, object]:
        sr = torch.nn.functional.interpolate(lr, size=target_size, mode="bilinear", align_corners=False)
        sr = self.net(sr)
...
class TinyDiscriminator(nn.Module):
    ...
    def forward(self, image: torch.Tensor, condition: torch.Tensor) -> dict[str, torch.Tensor]:
        condition = torch.nn.functional.interpolate(condition, size=image.shape[-2:], mode="bilinear", align_corners=False)
...
def _config(tmp_path):
    return {
        ...
        "optimizer": {
            "generator": {"type": "adamw", "lr": 1e-3, "betas": [0.0, 0.99], "weight_decay": 0.0},
            "discriminator": {"type": "adamw", "lr": 1e-3, "betas": [0.0, 0.99], "weight_decay": 0.0},
        },
        "loss": {
            "lambda_adv": 0.1,
            "lambda_pixel": 1.0,
...
def test_cpu_tiny_training_step_checkpoint_and_no_nan(tmp_path):
    ...
    trainer = Trainer(generator, discriminator, _loader(), config=_config(tmp_path), device="cpu")
    ...
    assert all(torch.isfinite(parameter).all() for parameter in discriminator.parameters())
    ...
    for saved_parameter, restored_parameter in zip(trainer.generator.parameters(), restored_g.parameters(), strict=True):
